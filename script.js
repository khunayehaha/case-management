...
const confirmBorrowReturnBtn = document.getElementById('borrowReturnBtn'); // Changed ID to match HTML
...
// For Vercel/Netlify, the API endpoint for `api/cases.js` will be `/api/cases`
const API_BASE_URL = ''; // Keep empty, Vercel/Netlify will handle routing
...
async function fetchCases() {
    try {
        // Netlify Functions are served from /api/{function-name}
...
async function openBorrowReturnModal(id, type) {
    try {
        ...
        if (type === 'borrow') {
            ...
            borrowReturnBtn.textContent = 'ยืนยันการเบิก'; // Changed ID to match HTML
            borrowerNameInput.value = '';
            borrowerNameInput.placeholder = 'กรอกชื่อผู้เบิก';
        } else { // type === 'return'
            borrowReturnModalTitle.textContent = 'บันทึกการคืนแฟ้มคดี';
            borrowReturnBtn.textContent = 'ยืนยันการคืน'; // Changed ID to match HTML
